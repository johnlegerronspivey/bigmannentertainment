...
# IPI Numbers Integration Endpoints (Direct implementation to avoid circular imports)
from industry_models import IPINumber, BIG_MANN_IPI_NUMBERS
from industry_service import IndustryIntegrationService

@api_router.get("/industry/ipi")
async def get_ipi_numbers(
    ...
    try:
        ...
        logging.error(f"Error retrieving IPI numbers: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to retrieve IPI numbers: {str(e)}")

@api_router.post("/industry/initialize")
async def initialize_industry_connections(admin_user: User = Depends(get_admin_user)):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Successfully initialized {total_partners} industry partners including IPI numbers",
        ...
        logging.error(f"Error initializing industry connections: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to initialize industry connections: {str(e)}")

@api_router.get("/industry/ipi/{ipi_number}")
...
@api_router.get("/industry/ipi/dashboard")
