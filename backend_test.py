...
class BackendTester:
    ...
    def test_api_base_url_configuration(self) -> bool:
        ...
        try:
            ...
            expected_base_url = "https://bme-media-hub.preview.emergentagent.com/api"
...
<TestMessage xmlns="http://ddex.net/xml/ern/41">
    ...
    def test_payment_checkout_session_creation(self) -> bool:
        ...
        try:
            ...
            params = {
                "package_id": "basic",
                "origin_url": "https://bme-media-hub.preview.emergentagent.com"
            }
    ...
    def test_email_service_initialization(self) -> bool:
        ...
        try:
            ...
            # We can't directly test the EmailService class, but we can test endpoints that use it
            
            # Check if email configuration endpoints exist (if any)
            # For now, we'll test indirectly through password reset functionality
            
            # Test forgot password endpoint exists and responds
    ...
    def test_email_configuration_validation(self) -> bool:
        ...
        try:
            # Test that the system can handle email configuration
            # We'll test this by checking if the forgot password endpoint works
    ...
    def test_smtp_settings_validation(self) -> bool:
        ...
        try:
            # Test SMTP settings by attempting password reset with existing user
            if not self.auth_token:
                # Need to ensure we have a user first
                if not self.test_user_registration() or not self.test_user_login():
                    self.log_result("email_service_validation", "SMTP Settings", False, "Could not create test user")
                    return False
    ...
    def test_password_reset_email_flow(self) -> bool:
        ...
        try:
            ...
            if not self.auth_token:
                if not self.test_user_registration() or not self.test_user_login():
                    self.log_result("password_reset_email", "Password Reset Email Flow", False, "Could not create test user")
                    return False
    ...
    def test_password_reset_graceful_degradation(self) -> bool:
        ...
        try:
            # Test with non-existent email (should still work gracefully)
    ...
    def test_registration_welcome_email(self) -> bool:
        ...
        try:
            # Test user registration to trigger welcome email
            from datetime import datetime, timedelta
    ...
    def test_email_template_branding(self) -> bool:
        ...
        try:
            # Test password reset to check template
            forgot_data = {"email": TEST_USER_EMAIL}
            response = self.make_request('POST', '/auth/forgot-password', json=forgot_data)
    ...
    def test_email_template_html_structure(self) -> bool:
        ...
        try:
            # Test that the system is configured for HTML emails
            # We can infer this from the response structure and fallback behavior
    ...
    def test_email_fallback_text_content(self) -> bool:
        ...
        try:
            # Test email system configuration for text fallback
            forgot_data = {"email": TEST_USER_EMAIL}
            response = self.make_request('POST', '/auth/forgot-password', json=forgot_data)
    ...
    def test_email_error_handling_missing_credentials(self) -> bool:
        ...
        try:
            # Test system behavior with potentially missing email credentials
            forgot_data = {"email": TEST_USER_EMAIL}
            response = self.make_request('POST', '/auth/forgot-password', json=forgot_data)
    ...
    def test_core_functionality_without_email(self) -> bool:
        ...
        try:
            # Test that registration works even if welcome email fails
            from datetime import datetime, timedelta
    ...
    def run_email_functionality_tests(self):
        ...
        print(f"\n📊 EMAIL FUNCTIONALITY TESTS SUMMARY:")
        email_categories = [
            'email_service_validation', 'password_reset_email', 'registration_welcome_email',
            'admin_notification_endpoints', 'email_template_validation', 'email_error_handling'
        ]
        
        total_email_passed = sum(self.results[cat]['passed'] for cat in email_categories)
        total_email_failed = sum(self.results[cat]['failed'] for cat in email_categories)
        
        print(f"✅ Passed: {total_email_passed}")
        print(f"❌ Failed: {total_email_failed}")
        
        if total_email_failed > 0:
            print(f"\n❌ Failed Email Tests Details:")
