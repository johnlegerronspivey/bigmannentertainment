...
# Test configuration
BACKEND_URL = os.environ.get('REACT_APP_BACKEND_URL', 'https://media-distributor.preview.emergentagent.com')
...
class SocialMediaStrategyTester:
    ...
    async def test_content_recommendations_endpoint(self):
        ...
        for test_case in test_cases:
            success, response, status = await self.make_request('POST', '/content-recommendations', test_case["data"])
    ...
    async def test_campaign_creation_endpoint(self):
        ...
        for test_campaign in test_campaigns:
            success, response, status = await self.make_request('POST', '/cross-promotion/campaign', test_campaign["data"])
